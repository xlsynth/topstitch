...
use crate::{mod_def::parser_port_to_port, mod_def::parser_param_to_param, ModDef, ModDefCore, ParserConfig, Usage, IO};
...
impl ModDef {
    ...
    pub fn parameterize<T: Into<BigInt> + Clone>(
        ...
        let parameters_with_string_values: Vec<(&str, String)> = bigint_params.iter().map(
            |(name, value)| {
